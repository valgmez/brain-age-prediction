...
# load them
ages = np.load('/nfshome/store01/users/c.c1732102/ages.npy', allow_pickle=True)
mris = np.load('/nfshome/store01/users/c.c1732102/mris.npy', allow_pickle=True)

# clean up data and ensure in correct format
# 80/20 train test split
# reshape
X_train, X_test, y_train, y_test = train_test_split(
    mris, ages, test_size=0.15, random_state=13)
...
for train, test in kfold.split(X_train, y_train):
    ...
    # 3. train the model
    history = brainAgeModel.fit(X_train[train], y_train[train], epochs=200,
                                batch_size=4, validation_data=(X_train[test], y_train[test]))
    ...
    cv_scores.append(test_eval[2])
print("Cross Validation MAE Mean: ", np.mean(
    cv_scores), " +/- ", np.std(cv_scores))
